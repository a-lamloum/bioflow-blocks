...
- [ ] T037 Wire mission completion in `src/app/page.tsx`: after successful simulation, call `checkCompletion(nodes, simulationResult)` and update `MissionState.completed`; when `completed` transitions to `true`, replace the step counter pill in the mission strip with a `Badge` (variant: success, text: "Mission complete!") and apply a 400ms teal-500 background tint to the strip (`motion-safe:` only, ease-out-quart); no modal; pass `missionState` + `onStepAdvance` to `MissionPanel`; advance `currentStepIndex` as learner adds each required block type to the canvas
